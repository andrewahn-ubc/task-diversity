...
jobs, each limited to one hour. Fifteen minutes before each limit, the run
requests a checkpoint after any in-progress evaluation or optimizer update
finishes. The checkpoint contains the policy, optimizer, CBP, environment,
recurrent state, and random generators. An intermediate chunk exits
successfully and the next corresponding array element restores the same
trajectory; it is not a new seed or policy. `aftercorr` dependencies let
different policies advance through their chains independently. If a run
remains incomplete at the final chunk, that array element fails so the gate or
aggregation cannot accept a result that exceeded its time budget.

The smoke chain contains 8 chunks. After reserving fifteen minutes per chunk
for a safe shutdown, it offers 6 hours against a 3-hour-40-minute planning
estimate; the exact 1.5x target is 5 hours 30 minutes. The main chain contains
21 chunks and offers 15 hours 45 minutes against a 10-hour estimate; the exact
1.5x target is 15 hours. Rounding up this way keeps the effective safety
allowance above 1.5x despite repeated startup, diagnostic, and checkpoint
overhead. The 25 primary conditions remain arrays, so elapsed time is governed
by the slowest run rather than the sum of all seeds when capacity is available.
The smoke arrays run one full-budget phase for each seed-0 condition. Those
five checkpoints are the corresponding main runs, so successful smoke
computation is reused rather than discarded.
